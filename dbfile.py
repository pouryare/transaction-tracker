...
class OneUser:
    ...
    def check_balance(self, uid, upikey):
        """Check account balance with UPI validation"""
        try:
            if self.upi != upikey:
                return -1
            self.curs.execute(
    ...
    def search_friend(self, value):
        """Search for a friend by name or phone"""
    ...
    def friend_info(self, value):
        """Get friend's account and profile photo"""
        self.curs.execute(
    ...
    def clear_recent(self, uid):
        """Clear recent transaction history"""
        self.curs.execute(
    ...
    def send_otp(self, email):
        ...
        try:
            ...
            message = f"Your OTP for SecureBank USA is {str(self.randnumber)}. If not done by you, please report immediately."
            print(message)
    ...
    def get_details_email(self):
        """Get user details by email (used in OTP flow)"""
